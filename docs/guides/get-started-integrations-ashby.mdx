...
1. Navigate to the [Integration API Keys page](https://app.ashbyhq.com/admin/api/keys?step=3).
    - Create a new API Key. You need to label it, you could label it as 'Embloy Integration Key'. You don't need to specify an Integration Partner but you could specify 'Embloy Platforms'
        ![Ashby Create Key](/img/integrations/ashby/ashby-1.png)
    - Activate the following modules:
        ![Ashby Key Settings](/img/integrations/ashby/ashby-2.png)
...
1. Navigate to the ['Secrets' Settings page](https://genius.embloy.com/settings?tab=secrets).
    ...
    ![Embloy Create Key](/img/integrations/ashby/ashby-3.png)
2. Confirm the Secret is set correctly:
    - In the filter input, enter the name of the secret you previously created
    - Ensure the displayed secret meets these criteria: A) Status is "Active" B) Type is "API Key" and C) Issued by "Ashby"
    - If any of these conditions are not met, select the secret and click "Delete Token(s)" in the toolbar.
    ![Embloy Check Key](/img/integrations/ashby/ashby-4.png)

    
3. Navigate to the ['Integrations' Settings page](https://genius.embloy.com/settings?tab=integrations).
    - Verify that the the Ashby Integration is shown as active
    - Hit 'Synchronize'
    - Hit '(Re)set Webhooks'
    ![Embloy Set & Sync](/img/integrations/ashby/ashby-5.png)
...
Let candidates know they can use Embloy to apply for your jobs! Update your [job templates](https://app.ashbyhq.com/jobs/templates) and your existing [job postings](https://app.ashbyhq.com/jobs/all) with the following text that links to `https://apply.embloy.com/ashby`: 
...
![Ashby Update posing](/img/integrations/ashby/ashby-6.png)
